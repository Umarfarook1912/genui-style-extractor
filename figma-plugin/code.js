...
function extractStyles(node) {
    const styles = {
        ...
        layout: 'layoutMode' in node ? extractLayout(node) : null,
        // Constraints
 ...
 */
function figmaToCSS(figmaStyles) {
    const css = {};
    // Dimensions
    if (figmaStyles.width)
        css.width = `${figmaStyles.width}px`;
    if (figmaStyles.height)
        css.height = `${figmaStyles.height}px`;
    ...
    if (figmaStyles.typography) {
        ...
        if (typo.fontSize)
            css.fontSize = `${typo.fontSize}px`;
        if (typo.fontName)
            css.fontFamily = typo.fontName.family;
    ...
    if (figmaStyles.layout && figmaStyles.layout.mode !== 'NONE') {
        ...
        // Padding
        if (figmaStyles.layout.paddingTop)
            css.paddingTop = `${figmaStyles.layout.paddingTop}px`;
        if (figmaStyles.layout.paddingRight)
            css.paddingRight = `${figmaStyles.layout.paddingRight}px`;
        if (figmaStyles.layout.paddingBottom)
            css.paddingBottom = `${figmaStyles.layout.paddingBottom}px`;
        if (figmaStyles.layout.paddingLeft)
            css.paddingLeft = `${figmaStyles.layout.paddingLeft}px`;
        // Alignment
